...
format:
	...
	docker compose run --no-deps --workdir / countries-informer-app /bin/bash -c "black src docs/source/*.py; isort --profile black src docs/source/*.py"
...
lint:
	...
	docker compose run --no-deps --workdir / countries-informer-app /bin/bash -c "pylint src; flake8 src; mypy src; black --check src"
